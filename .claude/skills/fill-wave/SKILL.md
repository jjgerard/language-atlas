---
name: fill-wave
description: Run a research wave that fills blank fields on the language-atlas maps — choosing the gap with gaps.js, building a worklist, drafting bullets under the content rules, gating every quote against the live source with terr-verify.js, and applying what passes with w4apply.js. ANY STAGE of that counts, not only the start — gating or verifying already-drafted units before they go near a data file, running w4apply on a prefix (dry or for real), or applying what survived a gate. A request naming only the verify or apply step is still this skill. Use it for filling in entries, researching countries or regions, adding content to a map, closing coverage gaps, or finding what is still empty — including "complete the entries for Asia", "we need dischargeCriteria filled", "the German Länder rows are empty, fill them", "I've got 12 drafted units, check them", "research these countries".
---

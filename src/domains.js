...
const DOMAINS = [
  ...
  },
  {
    ...
    live: true,
    fields: [
      ['inventory', 'How many languages', 'series', 'ONE row: how many languages the place has. `value` the number, `unit` count, `counted` what was counted in the counter’s own words, `basis` how it was produced — a reference catalogue, a census or an estimate. Leave `year` blank unless the source states its version or date; how many languages the school system engages with is the Languages field, not this one.'],
  ...
  },
  {
    ...
    fields: [
      ...
      ['linguistics', 'Linguistics', 'programme', 'One row per programme PER INSTITUTION where the institution is known: its name, a url to the programme page, and the level it reaches — bachelor, master or doctorate. Where a source gives only a total, one row with `institutions` set and `institution` blank is right. `orientation` records how the programme describes ITSELF, in one of its own words: generative, usage-based, functional or formal. It stays blank unless the department says so in its own words — never inferred from a reading list or a staff page, and never translated from one of these words into another, since a department calling itself functional has not called itself usage-based. Never estimate a count.'],
